...
const STATUS_CLS = {
  completed: 'border-cipher-500/40 text-cipher-400 bg-cipher-500/10',
  running:   'border-amber-400/40 text-amber-400 bg-amber-400/10',
  failed:    'border-rose-500/40 text-rose-400 bg-rose-500/10',
  pending:   'border-mist-700 text-mist-500',
}
...
export default function Runs() {
  ...
  useEffect(() => {
    Promise.all([api.train.experiments(), api.compare.all()])
      .then(([e, c]) => {
        setRuns(e.experiments || [])
        setMetrics(c.metrics || [])
        if (e.experiments?.length) setSel(e.experiments[0].id)
      })
  ...
  useEffect(() => {
    ...
    api.compare.rounds(sel).then(d => setRounds(r => ({...r, [sel]: d.rounds||[]}))).catch(()=>{})
  }, [sel])

  const filtered = useMemo(() => {
    let list = [...runs]
    if (search) list = list.filter(r =>
      r.algorithm.includes(search.toLowerCase()) || r.id.includes(search) || r.status.includes(search))
        ...
        <div className="panel p-0 overflow-hidden h-fit">
          <div className="divide-y divide-ink-700/60 max-h-[600px] overflow-y-auto scrollbar-thin">
            {filtered.length === 0 ? (
              ...
              return (
                <button key={run.id} onClick={()=>setSel(run.id)}
                  className={`w-full text-left px-4 py-3.5 transition border-l-2 ${isSel?'bg-signal-500/10 border-signal-500':'hover:bg-ink-800/50 border-transparent'}`}>
